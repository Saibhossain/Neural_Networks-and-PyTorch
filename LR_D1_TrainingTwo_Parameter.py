...
def train_model(iter):
    ...
    for epoch in range(iter):
        Yhat = forward(X)           # make a prediction
        loss = criterion(Yhat, Y)   # calculate the loss
        get_surface.set_para_loss(w.data.tolist(), b.data.tolist(), loss.tolist())  # Section for plotting
        if epoch % 3 == 0:
            get_surface.plot_ps()
        LOSS.append(loss)           # store the loss in the list LOSS
